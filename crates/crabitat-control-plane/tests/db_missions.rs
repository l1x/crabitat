...
fn setup_repo_and_issue(conn: &Connection) -> crabitat_control_plane::models::repos::Repo {
...
fn test_mission_status_recalculation() {
    ...
    let t1 = tasks::insert_task(&conn, &mission.mission_id, "step1", 0, "p1", 3, "queued").unwrap();
    let t2 = tasks::insert_task(&conn, &mission.mission_id, "step2", 1, "p2", 3, "queued").unwrap();
